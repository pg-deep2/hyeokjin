#models에서 모델 호출 및 학습. loss func, optimizer등 바꿔줌
...
##### Load your own model
from models.model import C3D, GRU


def denorm(x):
    out = (x + 1) / 2
    return out.clamp(0, 1) #min~max : 0~1
        ...
        for epoch in range(self.epochs):
            ...
            for step, (h, r) in enumerate(zip(self.h_loader,self.r_loader)):
                ...
                print('[%d/%d][%d/%d] - time: %.2f, h_loss: %.3f, r_loss: %.3f'
                      % (epoch+1, self.epochs, step+1, min(len(self.h_loader),len(self.r_loader)),
